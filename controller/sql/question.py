...
class Question:
    ...
    def add(self):
        ...
        if self.id is None:
            ...
            insert = ("INSERT INTO question (created_by, language, type, difficulty, prev_question_id, version_number, last_given, content, active) VALUES (%s, '%s', '%s', %s, %s, %s, '%s', '%s', %s);" % (self.created_by.id, self.language, self.atype, self.difficulty, (self.prev_question.id if type(prev_question) is Question else "Null"), self.version_number, self.last_given, self.content, self.active))
